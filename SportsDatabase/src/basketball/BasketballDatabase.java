...
public class BasketballDatabase extends Database {
	...
	//TODO MAYBE* implement some of the constructor behaviors 
	public BasketballDatabase(String path, String name) throws IllegalArgumentException {
		super(path, name);
		createFolder();
	}
	
	protected BasketballDatabase(String pathAndName) throws IllegalArgumentException {
		super(pathAndName);
		createFolder();
	}
	
	protected BasketballDatabase(Path pathAndName) throws IllegalArgumentException {
		super(pathAndName);
	}
	
	public BasketballDatabase(File f) throws IllegalArgumentException {
		super(f);
	}
	...
	public void createDataset(String dsName) throws IllegalArgumentException{
		...
		case "__g": new BasketballGameDataset(getPath() + getName() + "\\", dsName); break;
...
//		case "__c": new BasketballCoachDataset(getPath() + getName() + "\\", dsName); break;
	...
	public <T extends Dataset> T getDataset(String dsName) {
		switch(dsName.substring(dsName.length() - 3)) { // make sure that I shouldn't remove 3 
		case "__g": return (T) new BasketballGameDataset(new File(getPath() + getName() + "\\" + dsName)); 
...
//		case "__c": return new BasketballCoachDataset(new File(getPath() + getName() + "\\" + dsName));
	...
	protected void verifyName() throws IllegalArgumentException {
		if(!getName().endsWith("__b")) throw new IllegalArgumentException();
	}
